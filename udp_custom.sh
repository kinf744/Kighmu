...
print_center() { local msg="$*"; local cols=$(tput cols 2>/dev/null || echo 80); printf "%*s\n" $(( (${#msg} + cols) / 2 )) "$msg"; }
...
# Exclusion de ports interactive
setup_exclude_ports() {
    echo ""
...
check_os() {
    ...
    case "$ID" in
        ubuntu)
            (( ${VERSION_ID%%.*} < 20 )) && msg_error "Ubuntu 20.04+ requis"
            ;;
        debian)
            (( ${VERSION_ID%%.*} < 12 )) && msg_error "Debian 12+ requis"
            ;;
        *)
...
download_binaries() {
    ...
    chmod +x "$UDP_BIN"
    ln -sf "$UDP_BIN" /usr/local/bin/udp-custom  # Création lien pour éviter Script introuvable
