...
afficher_modes_ports() {
    ...
    if systemctl is-active --quiet ws-dropbear.service || pgrep -f ws-dropbear >/dev/null 2>&1; then any_active=1; fi
    ...
    if systemctl is-active --quiet ws-dropbear.service || pgrep -f ws-dropbear >/dev/null 2>&1 || screen -list | grep -q ws_wssr; then
