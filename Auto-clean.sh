...
# Format de sortie Xray : protobuf text
...
collect_v2ray_traffic() {
    ...
    # Trafic SSH : répartir entre les users SSH actifs
    if (( ssh_up + ssh_down > 0 )) && [[ -f "$USER_FILE" ]]; then
        # Compter les users SSH actifs (dans users.list et existants système)
        local ssh_users=()
        ...
        if (( nb_users > 0 )); then
            ...
            local share_down=$(( ssh_down / nb_users ))
            echo "[$(TS)] [V2RAY] SSH inbound ↑${ssh_up}B ↓${ssh_down}B → réparti sur $nb_users user(s)" >> "$LOG_FILE"
            for uname in "${ssh_users[@]}"; do
                [[ $first -eq 0 ]] && json+=","
                json+="{\"username\":\"${uname}\",\"upload_bytes\":${share_up},\"download_bytes\":${share_down}}"
                first=0
            done
        fi
    fi
