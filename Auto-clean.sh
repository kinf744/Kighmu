...
# ============================================================
# AUTO-MIGRATION SSH IPTABLES
# Vérifie et crée automatiquement la chaîne KIGHMU_SSH
# et les règles pour chaque user SSH existant.
# S'exécute silencieusement à chaque lancement — idempotent.
# ============================================================
DELTA_DIR="/var/lib/kighmu/ssh-counters"
USER_FILE="/etc/kighmu/users.list"

auto_migrate_ssh_iptables() {
    command -v iptables &>/dev/null || return
    [[ ! -f "$USER_FILE" ]] && return

    mkdir -p "$DELTA_DIR"

    # Créer la chaîne KIGHMU_SSH si absente
    if ! iptables -L KIGHMU_SSH 2>/dev/null | grep -q "Chain KIGHMU_SSH"; then
        iptables -N KIGHMU_SSH 2>/dev/null || true
        iptables -C OUTPUT -j KIGHMU_SSH 2>/dev/null \
            || iptables -A OUTPUT -j KIGHMU_SSH
        echo "[$(TS)] [SSH-MIGRATE] Chaine KIGHMU_SSH creee" >> "$LOG_FILE"
    fi

    # Créer les règles manquantes pour chaque user SSH
    while IFS='|' read -r username _rest; do
        ...
        # Règle OUTPUT déjà présente ?
        if iptables -L KIGHMU_SSH -n 2>/dev/null | grep -q "uid-owner ${uid}"; then
            # Règle présente — initialiser delta si fichier absent (ex: après reboot)
            [[ ! -f "${DELTA_DIR}/${username}.out" ]] && echo "0" > "${DELTA_DIR}/${username}.out"
            [[ ! -f "${DELTA_DIR}/${username}.in"  ]] && echo "0" > "${DELTA_DIR}/${username}.in"
            continue
        fi

        # Créer règle OUTPUT (download client)
        iptables -A KIGHMU_SSH -m owner --uid-owner "$uid" -j ACCEPT

        # Créer règle CONNMARK OUTPUT (marquer nouvelles connexions)
        iptables -A OUTPUT -m owner --uid-owner "$uid" \
            -m state --state NEW -j CONNMARK --set-mark "$uid"

        # Créer règle INPUT (upload client via mark)
        iptables -A INPUT -m connmark --mark "$uid" -j ACCEPT

        # Initialiser les fichiers delta à la valeur actuelle
        # (évite de compter le trafic passé comme nouveau)
        local cur_out cur_in hex_mark
        cur_out=$(iptables -nvx -L KIGHMU_SSH 2>/dev/null \
            | awk -v u="$uid" '
                /uid-owner/ && ($0 ~ "uid-owner " u " " || $0 ~ "uid-owner " u "$") {sum+=$2}
                END {print sum+0}')
        hex_mark=$(printf '0x%x' "$uid")
        cur_in=$(iptables -nvx -L INPUT 2>/dev/null \
            | awk -v m="$hex_mark" '/connmark/ && $0 ~ "mark match " m {sum+=$2} END{print sum+0}')

        echo "$cur_out" > "${DELTA_DIR}/${username}.out"
        echo "$cur_in"  > "${DELTA_DIR}/${username}.in"

        echo "[$(TS)] [SSH-MIGRATE] Regles creees pour $username (uid=$uid)" >> "$LOG_FILE"
    done < "$USER_FILE"
...
# Lancer la migration automatique au démarrage du script
auto_migrate_ssh_iptables
...
collect_xray_traffic() {
    ...
    if [[ $first -eq 0 ]]; then
        ...
        echo "[$(TS)] [XRAY] Aucun trafic" >> "$LOG_FILE"
    fi
}
...
# ── 0c. SSH : delta via KIGHMU_SSH + CONNMARK ────────────────
collect_ssh_traffic() {
    [[ ! -f "$USER_FILE" ]] && return
    command -v iptables &>/dev/null || return

    if ! iptables -L KIGHMU_SSH 2>/dev/null | grep -q "Chain KIGHMU_SSH"; then
        echo "[$(TS)] [SSH] Chaine KIGHMU_SSH absente" >> "$LOG_FILE"
        return
    fi
    ...
    if [[ $has_data -eq 1 ]]; then
        ...
        echo "[$(TS)] [SSH] Aucun trafic SSH" >> "$LOG_FILE"
    fi
}
