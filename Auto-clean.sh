...
# Download = sport (paquet sortant du serveur = client reçoit)
...
check_quota() {
    ...
    _block_zivpn() {
        ...
        [[ ! -f "$f" ]] && return
        mkdir -p "$blocked_dir"
        local user_line; user_line=$(grep "^${u}|" "$f" 2>/dev/null || true)
        if [[ -n "$user_line" ]]; then
        ...
        echo "[$(TS)] [BLOCK-ZIVPN] $u bloqué — password sauvegardé" >> "$LOG_FILE"
    }
    _block_hysteria() {
        ...
        echo "[$(TS)] [BLOCK-HYSTERIA] $u bloqué — password sauvegardé" >> "$LOG_FILE"
    }
