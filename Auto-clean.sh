...
# ── 0d. UDP Zivpn + Hysteria : lecture cumuls du service bandwidth ────────────
# Le service kighmu-bandwidth.sh accumule en temps réel dans
# BW_DIR/udp_zivpn_username.usage et BW_DIR/udp_hysteria_username.usage
collect_udp_traffic() {
    local BW_DIR="/var/lib/kighmu/bandwidth"
    local SENT_DIR="$BW_DIR/sent"
    mkdir -p "$SENT_DIR"
    ...
    for proto in "zivpn" "hysteria"; do
        local uf
        [[ "$proto" == "zivpn" ]]    && uf="/etc/zivpn/users.list"
        [[ "$proto" == "hysteria" ]] && uf="/etc/hysteria/users.txt"
        [[ ! -f "$uf" ]] && continue
        ...
        while IFS='|' read -r username _pass expire; do
            ...
            json+="{"username":"${username}","upload_bytes":${half},"download_bytes":${other}}"
            first=0; has_data=1
            echo "[$(TS)] [UDP-$proto] $username +$(( delta / 1048576 ))MB (cumul: $(( accumulated / 1048576 ))MB)" >> "$LOG_FILE"
    ...
    json+="]}"
    if [[ $has_data -eq 1 ]]; then
        echo "[$(TS)] [UDP] Envoi stats..." >> "$LOG_FILE"
        send_stats "$json"
    else
        echo "[$(TS)] [UDP] Aucun trafic UDP" >> "$LOG_FILE"
    fi
}
