...
# ============================================================
# SSH IPTABLES — comptage par IP source
# Pourquoi pas uid-owner ?
#   sshd, sshws, ws2_proxy tournent tous en root.
#   uid-owner ne peut pas distinguer les users SSH (tout = root).
# Solution : une règle iptables par IP cliente dans des chaînes
#   dédiées KIGHMU_IN / KIGHMU_OUT, mapping IP→user via auth.log.
# ============================================================
...
setup_ip_chains() {
    ...
    echo "[$(TS)] [SSH] Chaines KIGHMU_IN/OUT prets" >> "$LOG_FILE"
}
...
collect_xray_traffic() {
    ...
    raw=$("$XRAY_BIN" api statsquery --server="$XRAY_API" --reset 2>/dev/null) || return
...
collect_v2ray_traffic() {
    ...
    local raw
    # --reset remet les compteurs à 0 après lecture (évite le double-comptage)
    raw=$("$V2RAY_BIN" api statsquery --server="$V2RAY_API" --reset 2>/dev/null) || return
    [[ -z "$raw" ]] && return
...
# ── 0c. SSH : delta par IP source (auth.log + who + last) ────
collect_ssh_traffic() {
    [[ ! -f "$USER_FILE" ]] && return
    command -v iptables &>/dev/null || return

    # Construire map IP → username depuis sessions actives
    declare -A ip_user_map

    # Source 1 : auth.log (le plus fiable — toutes connexions récentes)
    ...
    if [[ -f "$authlog" ]]; then
        while IFS= read -r line; do
            if [[ "$line" =~ Accepted.*for[[:space:]]([^[:space:]]+)[[:space:]]from[[:space:]]([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+) ]]; then
                ...
                [[ "$auser" == "root" ]] && continue
                grep -q "^${auser}|" "$USER_FILE" 2>/dev/null && ip_user_map["$aip"]="$auser"
            fi
    ...
    while IFS= read -r line; do
        ...
        grep -q "^${wuser}|" "$USER_FILE" 2>/dev/null && ip_user_map["$wip"]="$wuser"
    done < <(who 2>/dev/null)

    # Source 3 : last (50 sessions récentes)
    while IFS= read -r line; do
        local luser lip
        luser=$(echo "$line" | awk '{print $1}')
        lip=$(echo "$line" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' | head -1)
        [[ -z "$luser" || -z "$lip" || "$luser" == "root" || "$luser" == "reboot" ]] && continue
        grep -q "^${luser}|" "$USER_FILE" 2>/dev/null && ip_user_map["$lip"]="$luser"
    done < <(last -n 50 2>/dev/null)

    if [[ ${#ip_user_map[@]} -eq 0 ]]; then
        echo "[$(TS)] [SSH] Aucune session SSH utilisateur detectee" >> "$LOG_FILE"
        return
    fi

    # Agréger les octets par username
    declare -A user_up user_down
    ...
    for ip in "${!ip_user_map[@]}"; do
        ...
        # Créer règles iptables pour cette IP si absentes
        ...
        cur_in=$(iptables  -nvx -L KIGHMU_IN  2>/dev/null | awk -v ip="$ip" '$0 ~ ip {sum+=$2} END{print sum+0}')
        cur_out=$(iptables -nvx -L KIGHMU_OUT 2>/dev/null | awk -v ip="$ip" '$0 ~ ip {sum+=$2} END{print sum+0}')
        ...
        # Delta avec protection reboot (compteur remis à 0)
