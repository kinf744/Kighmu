...
# Fonction générique de parsing statsquery (Xray ou V2Ray)
# $1 = binaire, $2 = adresse API, $3 = label log
_collect_vpn_stats() {
    local bin="$1" api="$2" label="$3"
    [[ ! -x "$bin" ]] && return
    ...
    [[ -z "$raw" ]] && { echo "[$(TS)] [$label] Aucune stat" >> "$LOG_FILE"; return; }
    ...
    if [[ $first -eq 0 ]]; then
        echo "[$(TS)] [$label] Envoi stats..." >> "$LOG_FILE"
        send_stats "$json"
    else
        echo "[$(TS)] [$label] Aucun trafic" >> "$LOG_FILE"
    fi
}
...
# ── 0b. V2Ray (vless-fastdns + SSH via dokodemo-door) ────────
# Le trafic SSH passe par V2Ray (dokodemo-door → port 22)
# donc ses stats apparaissent dans l'API V2Ray, pas iptables.
# Les users sont comptés via l'inbound tag "ssh" dans V2Ray.
collect_v2ray_traffic() {
    _collect_vpn_stats \
        "${V2RAY_BIN:-/usr/local/bin/v2ray}" \
        "${V2RAY_API:-127.0.0.1:10086}" \
        "V2RAY"
}
