...
init_udp_iptables() {
    ...
    if iptables -L KIGHMU_UDP_COUNT &>/dev/null 2>&1; then
        local nb_rules
        nb_rules=$(iptables -L KIGHMU_UDP_COUNT 2>/dev/null | grep -c "^udp" || true)
        if (( nb_rules >= 4 )); then
    ...
    iptables -A KIGHMU_UDP_COUNT -p udp --dport 6000:19999  2>/dev/null || true
    iptables -A KIGHMU_UDP_COUNT -p udp --sport 6000:19999  2>/dev/null || true
...
# Lire les bytes depuis KIGHMU_UDP_COUNT pour un port donné
# Additionne dport (entrant clients) + sport (sortant serveur) = total
read_udp_bytes_iptables() {
    local port="$1"
    local total=0
    # Additionner toutes les valeurs bytes pour ce port (dport + sport = 2 lignes)
    while IFS= read -r bytes; do
        [[ "$bytes" =~ ^[0-9]+$ ]] && total=$(( total + bytes ))
    done < <(
        iptables -nvx -L KIGHMU_UDP_COUNT 2>/dev/null \
        | grep -F "$port" \
        | awk '{print $2}'
    )
    echo "$total"
...
collect_udp_traffic() {
    ...
    if [[ -f "$zivpn_uf" ]]; then
        ...
        cur_zivpn=$(echo "$cur_zivpn" | tr -d ' \n\r')
        [[ ! "$cur_zivpn" =~ ^[0-9]+$ ]] && cur_zivpn=0
        ...
        # Calcul delta (reboot/reset protégé)
        local delta_zivpn=0
        if (( cur_zivpn >= prev_zivpn )); then
            delta_zivpn=$(( cur_zivpn - prev_zivpn ))
        else
            delta_zivpn=$cur_zivpn
        fi

        # RATTRAPAGE AUTOMATIQUE : détecte si du trafic n'a jamais été comptabilisé
        # (ex: script corrigé après une longue période de flush des compteurs).
        # Compare le compteur iptables réel à la somme de tout ce qui a été envoyé
        # au panel. Si l'écart dépasse 10 Mo, on injecte le delta manquant.
        ...
        local effective_zivpn=$(( delta_zivpn + recovery_zivpn ))
        echo "[$(TS)] [UDP-zivpn] compteur=$cur_zivpn prev=$prev_zivpn delta=$delta_zivpn recovery=$(( recovery_zivpn / 1048576 ))MB" >> "$LOG_FILE"
        ...
        if (( effective_zivpn > 0 )); then
            ...
            if (( nb > 0 )); then
                local share=$(( effective_zivpn / nb ))
                local half=$(( share / 2 ))
    ...
    if [[ -f "$hy_uf" ]]; then
        ...
        local delta_hy=0
        if (( cur_hy >= prev_hy )); then
            delta_hy=$(( cur_hy - prev_hy ))
        else
            delta_hy=$cur_hy
        fi

        # RATTRAPAGE AUTOMATIQUE (même logique que zivpn)
        local recovery_hy=0
        if (( cur_hy > 0 )); then
        ...
        echo "$cur_hy" > "$snap_hy"
        local effective_hy=$(( delta_hy + recovery_hy ))
        echo "[$(TS)] [UDP-hysteria] compteur=$cur_hy prev=$prev_hy delta=$delta_hy recovery=$(( recovery_hy / 1048576 ))MB" >> "$LOG_FILE"
