...
collect_v2ray_traffic() {
    ...
    # Comptage des sessions actives par user via /proc/PID/environ des processus sshd
    # (les connexions arrivent toutes de 127.0.0.1 donc iptables ne peut pas distinguer les users)
    if (( ssh_up + ssh_down > 0 )); then
        declare -A sess_count=()

        # Parcourir tous les PIDs sshd actifs
        while IFS= read -r pid; do
            [[ -z "$pid" ]] && continue
            # Lire le cmdline pour extraire le username
            # Format après auth complète : "sshd: USERNAME@pts/X" ou "sshd: USERNAME [priv]"
            local cmdline=""
            cmdline=$(tr '\0' ' ' < "/proc/$pid/cmdline" 2>/dev/null)
            local env_user=""
            if [[ "$cmdline" =~ ^sshd:[[:space:]]+([a-zA-Z0-9_.-]+)[@\[] ]]; then
                env_user="${BASH_REMATCH[1]}"
                # Ignorer root, le daemon listener et les sessions non-VPN
                [[ "$env_user" == "root" || "$env_user" == "/usr/sbin/sshd" ]] && continue
            else
                continue
            fi
            # Vérifier que c'est un user VPN connu (dans users.list)
            [[ -f "$USER_FILE" ]] && \
                grep -q "^${env_user}|" "$USER_FILE" 2>/dev/null || continue
            sess_count["$env_user"]=$(( ${sess_count["$env_user"]:-0} + 1 ))
        done < <(pgrep -x sshd 2>/dev/null)

        local total_sess=0
        for u in "${!sess_count[@]}"; do
            total_sess=$(( total_sess + sess_count[$u] ))
        done

        if (( total_sess > 0 )); then
            echo "[$(TS)] [V2RAY] SSH inbound ↑${ssh_up}B ↓${ssh_down}B → réparti par sessions (total=${total_sess})" >> "$LOG_FILE"
            for uname in "${!sess_count[@]}"; do
                local weight="${sess_count[$uname]}"
            ...
            # Aucune session identifiée : répartir sur tous les users connus dans users.list
            local ssh_users=()
