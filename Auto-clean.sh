...
init_udp_iptables() {
    # Créer la chaîne si absente, puis la vider et reconstruire
...
collect_udp_traffic() {
    ...
    if [[ -f "$zivpn_uf" ]]; then
        ...
        echo "[$(TS)] [UDP-zivpn] compteur=$cur_zivpn prev=$prev_zivpn delta=$delta_zivpn" >> "$LOG_FILE"
    ...
    if [[ -f "$hy_uf" ]]; then
        ...
        echo "$cur_hy" > "$snap_hy"
        echo "[$(TS)] [UDP-hysteria] compteur=$cur_hy prev=$prev_hy delta=$delta_hy" >> "$LOG_FILE"
