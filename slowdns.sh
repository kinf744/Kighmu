...
install_dependencies() {
    ...
    apt-get update -q
    apt-get install -y nftables wget curl jq python3 python3-venv python3-pip dnsutils netcat
}
...
choose_mode() {
    echo "1) AUTO (Cloudflare)"
    echo "2) MANUEL"
    read -rp "Choix mode [1-2]: " m
    case "$m" in
        1) MODE="auto" ;;
        2) MODE="man" ;;
        *) echo "Choix invalide"; exit 1 ;;
...
get_ns() {
    if [[ "$MODE" == "auto" ]]; then
        [[ -f "$CONFIG_FILE" ]] && { NS=$(cat "$CONFIG_FILE"); log "NS existant : $NS"; return; }
        NS=$(create_ns_cloudflare)
        echo "$NS" > "$CONFIG_FILE"
        log "NS créé : $NS"
    else
        read -rp "Entrez NS: " NS
        [[ -z "$NS" ]] && { echo "NS invalide"; exit 1; }
        echo "$NS" > "$CONFIG_FILE"
    fi
}
