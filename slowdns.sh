...
# --- Kernel tuning avancé pour stabilité ---
...
# --- Wrapper startup SlowDNS optimisé ---
...
log "Réglage MTU préféré à 1500 (fallback 1420)..."
if ip link set dev "$iface" mtu 1500 2>/dev/null; then
    log "MTU réglée à 1500"
else
    ip link set dev "$iface" mtu 1420 || true
    log "MTU fallback 1420 appliquée"
fi

NS=$(cat "$CONFIG_FILE" 2>/dev/null || echo "")
ssh_port=$(ss -tlnp | awk '/sshd/ {print $4; exit}' | sed -n 's/.*:$/\1/p')
[ -z "$ssh_port" ] && ssh_port=22

log "Démarrage du serveur SlowDNS..."
exec nice -n 0 "$SLOWDNS_BIN" -udp :$PORT -privkey-file "$SERVER_KEY" "$NS" 0.0.0.0:$ssh_port -max-udp-payload 512
EOF
chmod +x /usr/local/bin/slowdns-start.sh

# --- Service systemd SlowDNS optimisé ---
...
cat > /usr/local/bin/slowdns-monitor.sh <<'EOF'
#!/bin/bash
if ! pgrep -f dnstt-server >/dev/null; then
    systemctl restart slowdns.service
fi
EOF
chmod +x /usr/local/bin/slowdns-monitor.sh

# Ajouter une cron tous les 1min
(crontab -l 2>/dev/null; echo "* * * * * /usr/local/bin/slowdns-monitor.sh") | crontab -
