...
// ================================================================
// MYSQL via commande exec — aucune dépendance Go externe
// Lit les credentials depuis /opt/kighmu-panel/.env
// ================================================================
...
// mysqlQuery exécute une requête MySQL via la commande mysql et retourne le résultat
...
// mysqlDispo vérifie si MySQL est accessible
func mysqlDispo() bool {
	_, err := mysqlQuery("SELECT 1")
	return err == nil
}

// initDB vérifie la connexion MySQL au démarrage
func initDB() {
	if mysqlDispo() {
		fmt.Println("✅ MySQL accessible")
	} else {
		fmt.Println("⚠️ MySQL non disponible — mode fallback API activé")
	}
}

// queryUsageBytes retourne (upload+download) en bytes pour un username
...
func queryLimitGB(username string) int {
	query := fmt.Sprintf(
		`SELECT data_limit_gb FROM clients WHERE username = '%s'`, username)
	result, err := mysqlQuery(query)
	if err != nil || result == "" {
		return 0
	}
