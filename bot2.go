...
type UtilisateurV2Ray struct {
	...
	UUID    string
	Expire  string
	LimitGB int  // quota en Go (0 = illimité)
	UsedGB  float64 // consommé en Go (lu depuis v2ray_users.list)
}
...
func chargerUtilisateursV2Ray() {
	...
	for _, ligne := range lignes {
		...
		// Lire limitGB et usedBytes si présents (colonnes 4 et 5)
		limitGB := 0
		usedGB  := 0.0
		if len(parts) >= 4 {
		...
		if len(parts) >= 5 {
			if v, e := strconv.ParseFloat(strings.TrimSpace(parts[4]), 64); e == nil {
				// stocké en bytes → convertir en Go
				usedGB = v / 1073741824.0
			}
		}
...
func enregistrerUtilisateurV2Ray(u UtilisateurV2Ray) error {
	// Sauvegarder dans utilisateurs.json (pour V2Ray core)
	...
	// Sauvegarder dans v2ray_users.list (format: nom|uuid|expire|limitGB|usedBytes)
	// Lu par chargerUtilisateursV2Ray et traffic-collect.sh
	v2rayList := "/etc/kighmu/v2ray_users.list"
	f, errF := os.OpenFile(v2rayList, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0600)
	if errF != nil {
		return errF
	}
	defer f.Close()
	line := fmt.Sprintf("%s|%s|%s|%d|0\n", u.Nom, u.UUID, u.Expire, u.LimitGB)
...
func creerUtilisateurV2Ray(nom string, limitGB int, duree int) string {
	...
	if limitGB > 0 {
		builder.WriteString(fmt.Sprintf("➤ Quota     : %d Go\n", limitGB))
	} else {
		builder.WriteString("➤ Quota     : Illimité\n")
	}
...
// ================================================================
// CONSOMMATION XRAY — lecture stats API + users.json
// ================================================================

func lireStatsXrayAPI() map[string]int64 {
	...
	for _, line := range lines {
		...
		if strings.HasPrefix(line, "name:") {
			raw := strings.TrimPrefix(line, "name:")
			raw = strings.Trim(raw, "\\"")
			parts := strings.Split(raw, ">>>")
			if len(parts) >= 2 {
				currentTag = parts[1]
			}
		} else if strings.HasPrefix(line, "value:") && currentTag != "" {
			valStr := strings.TrimPrefix(line, "value:")
...
func mettreAJourUsageXray(statsAPI map[string]int64) {
	usersData, err := ioutil.ReadFile(xrayUsersFile)
	if err != nil {
		return
	}
	var js map[string][]map[string]interface{}
	if err := json.Unmarshal(usersData, &js); err != nil {
		return
	}
	for proto, clients := range js {
		for i, c := range clients {
			tag, _ := c["tag"].(string)
			if tag == "" {
				continue
			}
			if bytes, ok := statsAPI[tag]; ok && bytes > 0 {
				clients[i]["used_gb"] = float64(bytes) / 1073741824.0
			}
		}
		js[proto] = clients
	}
	newData, _ := json.MarshalIndent(js, "", "  ")
	ioutil.WriteFile(xrayUsersFile, newData, 0644)
}

func consommationXray() string {
	statsAPI := lireStatsXrayAPI()
	if len(statsAPI) > 0 {
		mettreAJourUsageXray(statsAPI)
	}
	liste := chargerUtilisateursXray()
	if len(liste) == 0 {
	...
	for i, u := range liste {
		usedGB := 0.0
		for _, c := range js[u.Proto] {
			uuid, _ := c["uuid"].(string)
			pw, _   := c["password"].(string)
			if uuid == u.UUID || pw == u.UUID {
				if v, ok := c["used_gb"].(float64); ok {
					usedGB = v
				}
				break
			}
		...
		if u.LimitGB <= 0 {
			if usedGB < 1.0 {
				consoStr = fmt.Sprintf("%.0f MB / ∞", usedGB*1024)
			} else {
				consoStr = fmt.Sprintf("%.2f GB / ∞", usedGB)
			}
		} else {
			pct := (usedGB / float64(u.LimitGB)) * 100.0
			if usedGB < 1.0 {
				consoStr = fmt.Sprintf("%.0f MB / %d GB (%.0f%%)", usedGB*1024, u.LimitGB, pct)
			} else {
				consoStr = fmt.Sprintf("%.2f / %d GB (%.0f%%)", usedGB, u.LimitGB, pct)
			}
			if pct >= 100.0 {
				consoStr = "🔴 " + consoStr + " DEPASSE"
			} else if pct >= 90.0 {
				consoStr = "🟡 " + consoStr
			} else {
				consoStr = "🟢 " + consoStr
...
func consommationV2Ray() string {
	...
	for i, u := range utilisateursV2Ray {
		...
		if u.LimitGB <= 0 {
			if u.UsedGB < 1.0 {
				consoStr = fmt.Sprintf("%.0f MB / ∞", u.UsedGB*1024)
			} else {
				consoStr = fmt.Sprintf("%.2f GB / ∞", u.UsedGB)
			}
		} else {
			pct := (u.UsedGB / float64(u.LimitGB)) * 100.0
			if u.UsedGB < 1.0 {
				consoStr = fmt.Sprintf("%.0f MB / %d GB (%.0f%%)", u.UsedGB*1024, u.LimitGB, pct)
			} else {
				consoStr = fmt.Sprintf("%.2f / %d GB (%.0f%%)", u.UsedGB, u.LimitGB, pct)
			}
			if pct >= 100.0 {
				consoStr = "🔴 " + consoStr + " DEPASSE"
			} else if pct >= 90.0 {
...
============================================`
			...
			keyboard := tgbotapi.NewInlineKeyboardMarkup(
				...
				tgbotapi.NewInlineKeyboardRow(
					tgbotapi.NewInlineKeyboardButtonData("📈 Conso Xray", "xray_conso"),
					tgbotapi.NewInlineKeyboardButtonData("📉 Conso V2Ray", "v2ray_conso"),
				),
				tgbotapi.NewInlineKeyboardRow(
