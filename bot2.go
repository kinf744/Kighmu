...
var (
	...
	panelDB   *sql.DB // connexion MySQL partagée
)
...
// ================================================================
// CONNEXION MYSQL — lit les credentials depuis /opt/kighmu-panel/.env
// ================================================================
func lireEnvValue(file, key string) string {
...
func initDB() {
	envFile := "/opt/kighmu-panel/.env"
	host   := lireEnvValue(envFile, "DB_HOST")
	port   := lireEnvValue(envFile, "DB_PORT")
	dbName := lireEnvValue(envFile, "DB_NAME")
	dbUser := lireEnvValue(envFile, "DB_USER")
	dbPass := lireEnvValue(envFile, "DB_PASSWORD")

	if host == "" { host = "127.0.0.1" }
	if port == "" { port = "3306" }
	if dbName == "" { dbName = "kighmu_panel" }

	dsn := fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?parseTime=true&timeout=5s",
		dbUser, dbPass, host, port, dbName)

	var err error
	panelDB, err = sql.Open("mysql", dsn)
	if err != nil {
		fmt.Println("⚠️ MySQL non disponible:", err)
		return
	}
	panelDB.SetMaxOpenConns(5)
	panelDB.SetConnMaxLifetime(time.Minute * 3)
	if err = panelDB.Ping(); err != nil {
		fmt.Println("⚠️ MySQL ping échoué:", err)
		panelDB = nil
		return
	}
	fmt.Println("✅ MySQL connecté")
}

// queryUsageBytes retourne (upload+download) en bytes pour un username depuis usage_stats
func queryUsageBytes(username string) int64 {
	if panelDB == nil {
		return -1 // signale que DB non dispo
	}
	var total int64
	err := panelDB.QueryRow(`
		SELECT COALESCE(SUM(u.upload_bytes + u.download_bytes), 0)
		FROM clients c
		LEFT JOIN usage_stats u ON u.client_id = c.id
		WHERE c.username = ?`, username).Scan(&total)
	if err != nil {
		return 0
	}
	return total
...
// queryLimitGB retourne data_limit_gb depuis clients pour un username
func queryLimitGB(username string) int {
	if panelDB == nil {
		return 0
	}
	var limit float64
	err := panelDB.QueryRow(
		`SELECT data_limit_gb FROM clients WHERE username = ?`, username).Scan(&limit)
	if err != nil {
		return 0
	}
	return int(limit)
...
// lireStatsXrayAPIMap interroge l'API Xray et retourne map[tag]bytes
func lireStatsXrayAPIMap() map[string]int64 {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		if strings.HasPrefix(line, "name:") {
			// ex: name:"user>>>vmess_alice_abc12345>>>traffic>>>uplink"
			raw := strings.Trim(strings.TrimPrefix(line, "name:"), `"`)
			parts := strings.Split(raw, ">>>")
			if len(parts) >= 2 {
				currentName = parts[1] // tag complet ex: vmess_alice_abc12345
			}
		} else if strings.HasPrefix(line, "value:") && currentName != "" {
			valStr := strings.Trim(strings.TrimPrefix(line, "value:"), `"`)
...
// bytesXrayParNom trouve le total bytes pour un nom d'user dans les stats Xray
// Le tag Xray est de la forme : proto_nom_uuid8 (ex: vmess_alice_abc12345)
func bytesXrayParNom(statsMap map[string]int64, nom string) int64 {
	var total int64
	for tag, bytes := range statsMap {
		// Le tag contient le nom après le premier underscore
...
func consommationXray() string {
	...
	// Pré-charger les stats API Xray si MySQL absent (évite 1 appel par user)
	var xrayStatsMap map[string]int64
	if !dbDispo {
	...
	for i, u := range liste {
		...
		if dbDispo {
			// Niveau 1 : MySQL (même données que le panel)
			totalBytes = queryUsageBytes(u.Nom)
			limitGB    = queryLimitGB(u.Nom)
			if limitGB == 0 { limitGB = u.LimitGB }
		} else {
			// Niveau 2 : API Xray directe (xray api statsquery)
			totalBytes = bytesXrayParNom(xrayStatsMap, u.Nom)
			limitGB    = u.LimitGB
			// Niveau 3 : fichier .usage local si API retourne 0
			if totalBytes == 0 {
...
func consommationV2Ray() string {
	...
	for i, u := range utilisateursV2Ray {
		...
		if dbDispo {
			...
			// Niveau 2 : API V2Ray + cumul local
