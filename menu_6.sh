...
# ============================================================
# FIX : safe_write vérifie que le fichier n'est pas vide
# ET que le JSON est valide avant d'écraser la destination
# ============================================================
safe_write() {
    local tmp_file="$1" dest_file="$2"

    # Vérification 1 : fichier non vide
    if [[ ! -s "$tmp_file" ]]; then
        echo -e "${RED}❌ Fichier temporaire vide — abandon de l'écriture dans $dest_file.${RESET}"
        rm -f "$tmp_file"
        return 1
    fi
    ...
    if ! jq . "$tmp_file" > /dev/null 2>&1; then
        ...
        echo -e "${YELLOW}   Contenu du fichier tmp :${RESET}"
        cat "$tmp_file"
        rm -f "$tmp_file"
        return 1
    fi
...
create_config() {
  ...
  # Vérifier que config.json existe et est valide avant de commencer
  if [[ ! -f "$CONFIG_FILE" ]]; then
      echo -e "${RED}❌ config.json introuvable : $CONFIG_FILE${RESET}"
      return 1
  fi
  if ! jq . "$CONFIG_FILE" > /dev/null 2>&1; then
      echo -e "${RED}❌ config.json est corrompu — impossible de créer l'utilisateur.${RESET}"
      echo -e "${YELLOW}   Vérifiez avec : jq . $CONFIG_FILE${RESET}"
      return 1
  fi
  ...
  # ── config.json — FIX : utilisation de |= (. // []) + [...] ────────────
  # L'ancienne syntaxe (.inbounds[]|select(…)|.settings.clients //= []) += [...]
  # produisait un fichier vide car jq ne supporte pas l'assignation composée
  # sur un chemin multi-valeurs. La forme correcte est |= ... + [...]
  local tmp_config
  ...
  # Vérification supplémentaire : s'assurer que le client a bien été ajouté
  local client_count_before client_count_after
  client_count_before=$(jq "[.inbounds[] | select(.protocol==\"${proto}\") | .settings.clients // [] | .[]] | length" "$CONFIG_FILE" 2>/dev/null || echo 0)
  client_count_after=$(jq  "[.inbounds[] | select(.protocol==\"${proto}\") | .settings.clients // [] | .[]] | length" "$tmp_config"   2>/dev/null || echo 0)

  if (( client_count_after <= client_count_before )); then
      echo -e "${RED}❌ Le client n'a pas été ajouté au config.json — abandon.${RESET}"
      rm -f "$tmp_config"
      return 1
  fi
  ...
  # Restart Xray avec vérification post-restart
  systemctl restart xray
  sleep 2
  if systemctl is-active --quiet xray; then
    echo -e "${GREEN}✅ Xray redémarré avec succès.${RESET}"
  else
    echo -e "${RED}❌ Xray n'a pas redémarré — vérifiez le config.json :${RESET}"
...
delete_user_by_number() {
    ...
    if systemctl is-active --quiet xray; then
      ...
      echo -e "${RED}❌ Xray n'a pas redémarré après suppression — vérifiez le config.json.${RESET}"
      journalctl -u xray -n 10 --no-pager
    fi
}
