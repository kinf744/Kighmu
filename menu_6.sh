...
delete_user_by_number() {
    ...
    if [[ "$sel_proto" == "trojan" ]]; then
      # Trojan identifié par password (= uuid) dans config.json
      jq --arg pw "$sel_uuid" \
        '(.inbounds[] | select(.protocol=="trojan") | .settings.clients)
         |= map(select(.password != $pw))' \
        "$CONFIG_FILE" > "$tmp_config"
    elif [[ "$sel_proto" == "vmess" ]]; then
      # VMess identifié par id — on ne touche QUE les inbounds vmess
      jq --arg u "$sel_uuid" \
        '(.inbounds[] | select(.protocol=="vmess") | .settings.clients)
         |= map(select(.id != $u))' \
        "$CONFIG_FILE" > "$tmp_config"
    else
      # VLESS identifié par id — on ne touche QUE les inbounds vless
      jq --arg u "$sel_uuid" \
