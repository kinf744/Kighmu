...
# Cloner le dépôt udp-request si non présent
if [ ! -d "$INSTALL_DIR" ]; then
    echo "Clonage du dépôt udp-request..."
    git clone https://github.com/prjkt-nv404/UDP-Request-Manager.git "$INSTALL_DIR"
else
...
# Vérifier la présence et droits du binaire
if [ ! -x "$BIN_PATH" ]; then
...
# Création ou modification du fichier config.json spécifique UDP Request
...
EOF
    ...
    echo "Modification de la configuration existante..."
...
# Ouverture des ports UDP principaux et plage dans iptables
...
echo "Démarrage du démon udp-request..."
