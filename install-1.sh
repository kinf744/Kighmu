...
do_install() {
  ...
  # Créer la table monthly_traffic_snapshot (snapshot trafic mensuel)
  mysql "${DB_NAME}" << 'SNAPSHOTSQL' 2>/dev/null || true
...
SNAPSHOTSQL
  ...
  # ── Patch server.js : injecter snapshotClientTraffic + snapshotResellerTraffic ──
  python3 << 'PATCHEOF' 2>/dev/null || true
import sys, re
path = "/opt/kighmu-panel/server.js"
...
async function snapshotResellerTraffic(resellerId) {
  try {
    ...
    console.log("[SNAPSHOT] Revendeur #" + resellerId + " sauvegardé");
...
if "snapshotClientTraffic" not in c:
    # Injecter avant cleanupReseller
    c = c.replace("async function cleanupReseller(resellerId) {", snapshot_fns + "async function cleanupReseller(resellerId) {")
    # Patcher cleanupReseller
    c = c.replace(
    ...
    with open(path, "w") as f:
        f.write(c)
    print("✅ server.js patché avec succès")
else:
    print("ℹ️  server.js déjà patché")
PATCHEOF
...
# ── XRAY : API JSON statsquery ──────────────────────────────
# L'API xray retourne du JSON : {"stat":[{"name":"...","value":1234},...]}
# Les entrées sans trafic n'ont pas de champ "value" — utiliser jq obligatoire
collect_xray() {
  [ ! -x "\$XRAY_BIN" ] && return
  local raw
  ...
  if ! command -v jq &>/dev/null; then
    echo "[\${TS}] [XRAY] jq manquant — impossible de parser les stats"
    return
  fi
  local json='{"stats":[' first=1
  declare -A up_map down_map
  while IFS= read -r entry; do
    local name value dir user
    name=\$(echo "\$entry"  | jq -r '.name  // ""')
    value=\$(echo "\$entry" | jq -r '.value // 0')
    [[ -z "\$name" || "\$name" == "null" ]] && continue
    [[ "\$name" != user\>\>\>*\>\>\>traffic\>\>\>* ]] && continue
    user=\$(echo "\$name" | sed 's/^user>>>\(.*\)>>>traffic>>>.*/\1/')
    dir=\$(echo "\$name"  | sed 's/.*>>>//')
    [[ "\$value" == "0" || -z "\$value" ]] && continue
    [[ "\$dir" == "uplink"   ]] && up_map["\$user"]=\$(( \${up_map["\$user"]:-0}   + value ))
    [[ "\$dir" == "downlink" ]] && down_map["\$user"]=\$(( \${down_map["\$user"]:-0} + value ))
  done < <(echo "\$raw" | jq -c '.stat[]? // empty')
  ...
  json+=']}' 
  [ \$first -eq 0 ] && { echo "[\${TS}] [XRAY] Envoi stats..."; send_stats "\$json"; } || echo "[\${TS}] [XRAY] Aucune stat utilisateur"
}
