...
INFOFILE
  ...
  # ── CRITIQUE : enregistrer le domaine pour server.js ─────────
  # server.js lit /etc/kighmu/domain.txt pour construire les
  # liens SSH WS/SSL/SlowDNS affichés cote client VPN (HTTP Injector)
  mkdir -p /etc/kighmu
  echo "${DOMAIN}" > /etc/kighmu/domain.txt
  chmod 644 /etc/kighmu/domain.txt
  ok "Domaine sauvegarde → /etc/kighmu/domain.txt"

  # Compatibilite menu1.sh : ecrire ~/.kighmu_info
  cat > ~/.kighmu_info << KIGHMUINFO
DOMAIN=${DOMAIN}
HOST_IP=$(hostname -I | awk '{print $1}')
KIGHMUINFO
  chmod 600 ~/.kighmu_info
  ok "Infos VPS → ~/.kighmu_info"

  # Ajouter DOMAIN dans .env si absent
