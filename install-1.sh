...
PKG
  ...
  if [[ $USE_TLS -eq 1 ]]; then
    ...
    # Chercher d'abord le dossier ECC exact, puis fallback RSA
    ...
    # Si pas encore de certificat → émettre via acme.sh
    if [[ -z "$CERT_FILE" ]]; then
...
NGINXTEMP
      ...
      # Ouvrir port 80 temporairement
      iptables -I INPUT -p tcp --dport 80 -j ACCEPT 2>/dev/null || true

      # Émettre le certificat ECC
    ...
    if [[ $USE_TLS -eq 1 && -n "$CERT_FILE" ]]; then
      # Config Nginx HTTPS finale sur port 8587 + redirect 8585→8587
...
NGINXSSL
      ACCESS_URL="https://${DOMAIN}:${NGINX_PORT_TLS}"
      # Configurer le renouvellement automatique acme.sh
      "${ACME_DIR}/acme.sh" --install-cert -d "${DOMAIN}" --ecc \
        ...
        >/dev/null 2>&1 || true
      ln -sf /etc/nginx/sites-available/kighmu /etc/nginx/sites-enabled/kighmu
      nginx -t 2>/dev/null && { systemctl start nginx; systemctl enable nginx; systemctl reload nginx; }
      ok "TLS activé → ${ACCESS_URL}"
    fi

    if [[ $USE_TLS -eq 0 ]]; then
      # Fallback HTTP si acme.sh a échoué
      cat > /etc/nginx/sites-available/kighmu << NGINXHTTP_FALLBACK
server {
    listen ${NGINX_PORT};
    listen [::]:${NGINX_PORT};
...
NGINXHTTP
  ...
  if command -v ufw &>/dev/null; then
    if [[ $USE_TLS -eq 1 ]]; then
      ...
      ok "Firewall : port ${NGINX_PORT_TLS} (HTTPS) + ${NGINX_PORT} (redirect) ouverts"
    else
      ufw allow ${NGINX_PORT}/tcp >/dev/null 2>&1 || true
      ufw deny  ${NODE_PORT}/tcp  >/dev/null 2>&1 || true
    ...
    if [[ $USE_TLS -eq 1 ]]; then
      ...
      ok "Firewall iptables : ports ${NGINX_PORT_TLS} et ${NGINX_PORT} ouverts"
    else
      iptables -I INPUT -p tcp --dport ${NGINX_PORT} -j ACCEPT 2>/dev/null || true
