...
' ' ')
    ...
    # ── Collecte trafic UDP Zivpn (port 5667) ──────────────────
    # Principe : règles iptables par username via fichier users.list
    # Zivpn : /etc/zivpn/users.list  format: username|password|expire
    # Hysteria : /etc/hysteria/users.txt  format: username|password|expire
    # On crée une règle iptables par username (mark = hash du username)
    # et on lit le delta comme pour SSH

    ZIVPN_UF="/etc/zivpn/users.list"
    HY_UF="/etc/hysteria/users.txt"

    # ── Helper : lire bytes iptables pour un mark ───────────────
    read_mark_bytes() {
        local mark="\$1"
        iptables -nvx -L KIGHMU_UDP 2>/dev/null \
            | awk -v m="\$mark" '\$0 ~ "mark match " m {sum+=\$2} END{print sum+0}'
    }

    # ── Initialiser la chaîne KIGHMU_UDP si absente ─────────────
    if ! iptables -L KIGHMU_UDP 2>/dev/null | grep -q "Chain KIGHMU_UDP"; then
        iptables -N KIGHMU_UDP 2>/dev/null || true
        iptables -C INPUT  -j KIGHMU_UDP 2>/dev/null || iptables -A INPUT  -j KIGHMU_UDP
        iptables -C OUTPUT -j KIGHMU_UDP 2>/dev/null || iptables -A OUTPUT -j KIGHMU_UDP
    fi

    # ── Fonction : collecter trafic UDP pour un fichier d'users ──
    collect_udp_file() {
        local uf="\$1"
        local proto="\$2"  # "zivpn" ou "hysteria"
        [[ ! -f "\$uf" ]] && return
        local today
        today=\$(date +%F)

        while IFS='|' read -r uname _pass expire; do
            [[ -z "\$uname" ]] && continue
            [[ "\$expire" < "\$today" ]] && continue  # expiré

            # Calculer un mark unique à partir du username (CRC simple)
            local mark
            mark=\$(printf '%d' "\$(echo -n "\$uname" | cksum | cut -d' ' -f1)")
            # Limiter à 32 bits positifs non nuls
            mark=\$(( (mark % 2147483647) + 1 ))
            local hex_mark
            hex_mark=\$(printf '0x%x' "\$mark")

            # Créer règles iptables INPUT/OUTPUT pour ce mark si absentes
            if ! iptables -L KIGHMU_UDP -n 2>/dev/null | grep -q "mark match \$hex_mark"; then
                # Marquer les paquets du port UDP correspondant au protocole
                if [[ "\$proto" == "zivpn" ]]; then
                    # Zivpn : port range 6000-19999
                    iptables -A INPUT  -p udp --dport 6000:19999 -m mark --mark 0 -j MARK --set-mark "\$mark" 2>/dev/null || true
                    iptables -A OUTPUT -p udp --sport 6000:19999 -m mark --mark 0 -j MARK --set-mark "\$mark" 2>/dev/null || true
                else
                    # Hysteria : port range 20000-50000
                    iptables -A INPUT  -p udp --dport 20000:50000 -m mark --mark 0 -j MARK --set-mark "\$mark" 2>/dev/null || true
                    iptables -A OUTPUT -p udp --sport 20000:50000 -m mark --mark 0 -j MARK --set-mark "\$mark" 2>/dev/null || true
                fi
                # Règles de comptage dans KIGHMU_UDP
                iptables -A KIGHMU_UDP -m mark --mark "\$mark" -j ACCEPT 2>/dev/null || true

                # Initialiser delta
                echo "0" > "\$BW_DIR/udp_\${proto}_\${uname}.last_in"
                echo "0" > "\$BW_DIR/udp_\${proto}_\${uname}.last_out"
            fi

            # Lire compteurs actuels
            local cur_in cur_out
            cur_in=\$(iptables -nvx -L INPUT 2>/dev/null \
                | awk -v m="\$hex_mark" '/mark match/ && \$0 ~ "mark match " m {sum+=\$2} END{print sum+0}')
            cur_out=\$(iptables -nvx -L OUTPUT 2>/dev/null \
                | awk -v m="\$hex_mark" '/mark match/ && \$0 ~ "mark match " m {sum+=\$2} END{print sum+0}')
            cur_in=\${cur_in:-0}; cur_out=\${cur_out:-0}

            # Lire précédentes valeurs
            local prev_in prev_out
            prev_in=\$(cat "\$BW_DIR/udp_\${proto}_\${uname}.last_in" 2>/dev/null); prev_in=\${prev_in:-0}
            prev_out=\$(cat "\$BW_DIR/udp_\${proto}_\${uname}.last_out" 2>/dev/null); prev_out=\${prev_out:-0}

            # Calcul delta (protection reboot)
            local d_in d_out
            (( cur_in  >= prev_in  )) && d_in=\$((  cur_in  - prev_in  )) || d_in=\$cur_in
            (( cur_out >= prev_out )) && d_out=\$(( cur_out - prev_out )) || d_out=\$cur_out

            # Sauvegarder valeurs actuelles
            echo "\$cur_in"  > "\$BW_DIR/udp_\${proto}_\${uname}.last_in"
            echo "\$cur_out" > "\$BW_DIR/udp_\${proto}_\${uname}.last_out"

            (( d_in + d_out == 0 )) && continue

            # Accumuler dans le cumul total
            local usagefile="\$BW_DIR/udp_\${proto}_\${uname}.usage"
            local accumulated=0
            [[ -f "\$usagefile" ]] && accumulated=\$(cat "\$usagefile" 2>/dev/null)
            [[ ! "\$accumulated" =~ ^[0-9]+\$ ]] && accumulated=0
            local new_total=\$(( accumulated + d_in + d_out ))
            echo "\$new_total" > "\$usagefile"

            # Accumuler pour envoi panel
            pending_up["\$uname"]=\$(( \${pending_up[\$uname]:-0} + d_in ))
            pending_down["\$uname"]=\$(( \${pending_down[\$uname]:-0} + d_out ))

        done < "\$uf"
    }

    collect_udp_file "\$ZIVPN_UF"   "zivpn"
    collect_udp_file "\$HY_UF"      "hysteria"
