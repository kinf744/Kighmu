...
PKG
  ...
  # Générer le hash bcrypt via fichier JS temporaire
  # (évite les problèmes avec les caractères spéciaux ! $ ' dans bash)
  cat > /tmp/kighmu_hash.js << 'JSEOF'
const b = require('/opt/kighmu-panel/node_modules/bcryptjs');
const pass = process.env.KIGHMU_PASS;
if (!pass) { process.stderr.write('KIGHMU_PASS vide\n'); process.exit(1); }
process.stdout.write(b.hashSync(pass, 12));
JSEOF
  ADMIN_HASH=$(KIGHMU_PASS="${ADMIN_PASS}" node /tmp/kighmu_hash.js 2>/dev/null || echo "")
  rm -f /tmp/kighmu_hash.js
  if [[ -n "$ADMIN_HASH" ]]; then
    ...
    warn "Hash bcrypt impossible — admin par défaut utilisé : admin / Admin2024"
    ADMIN_USER="admin"
    ADMIN_PASS="Admin2024"
    ADMIN_HASH=$(KIGHMU_PASS="Admin2024" node -e "const b=require('/opt/kighmu-panel/node_modules/bcryptjs');process.stdout.write(b.hashSync(process.env.KIGHMU_PASS,12));" 2>/dev/null || echo "")
    [[ -n "$ADMIN_HASH" ]] && mysql "${DB_NAME}" -e "DELETE FROM admins; INSERT INTO admins (username, password) VALUES ('admin', '${ADMIN_HASH}');" 2>/dev/null
  fi
...
NGINXHTTP
  ...
  # (hash admin déjà configuré à l'étape 7)
