...
async function sshAdd(username, password, expiryDate) {
  try {
    ...
    // Lire DOMAIN depuis ~/.kighmu_info si /etc/kighmu/domain.txt absent (comme menu1.sh)
    const getKighmuDomain = () => {
      const direct = readF('/etc/kighmu/domain.txt');
      if (direct) return direct;
      try {
        const info = fs.readFileSync(`${process.env.HOME || '/root'}/.kighmu_info`, 'utf8');
        const m = info.match(/^DOMAIN=(.+)$/m);
        return m ? m[1].trim() : '';
      } catch { return ''; }
    };

    // Création utilisateur avec home et shell bash (comme menu1.sh)
    await execAsync(`useradd -m -s /bin/bash ${username} 2>/dev/null || true`);
    // Utiliser printf pour éviter les problèmes avec les caractères spéciaux dans le mot de passe
    await execAsync(`printf '%s:%s\\n' '${username}' '${password.replace(/'/g, "'\\''")}' | chpasswd`);

    // Appliquer la date d'expiration du compte via chage (comme menu1.sh)
    await execAsync(`chage -E ${exp} ${username} 2>/dev/null || true`);

    // Créer le .bashrc avec affichage du banner au login (comme menu1.sh)
    ...
    try {
      ...
      fs.writeFileSync(`${userHome}/.bashrc`, bashrcContent);
      await execAsync(`chown ${username}:${username} ${userHome}/.bashrc 2>/dev/null || true`);
      fs.chmodSync(`${userHome}/.bashrc`, 0o644);
      await execAsync(`chown ${username}:${username} ${userHome} 2>/dev/null || true`);
    } catch (e3) { console.error('[SSH] Erreur .bashrc:', e3.message); }

    // Écriture dans /etc/kighmu/users.list
    try {
      ...
      const domain    = getKighmuDomain() || readF('/etc/xray/domain') || readF('/tmp/.xray_domain') || '';
    ...
    // Synchronisation ZIVPN (comme menu1.sh)
    try {
      const zivpnUserFile = '/etc/zivpn/users.list';
      const zivpnCfgFile  = '/etc/zivpn/config.json';
      if (fs.existsSync(zivpnCfgFile) && fs.existsSync(zivpnUserFile)) {
        let zlines = fs.readFileSync(zivpnUserFile, 'utf8').split('\n').filter(l => l.trim() && !l.startsWith(`${username}|`));
        zlines.push(`${username}|${password}|${exp}`);
        fs.writeFileSync(zivpnUserFile, zlines.join('\n') + '\n');
        fs.chmodSync(zivpnUserFile, 0o600);
        _zivpnSyncConfig(zivpnUserFile, zivpnCfgFile);
        await restartService('zivpn');
        console.log(`[SSH] ✅ ZIVPN synchronisé pour ${username}`);
      }
    } catch (ez) { console.error('[SSH] Erreur sync ZIVPN:', ez.message); }

    // Synchronisation Hysteria (comme menu1.sh)
    try {
      const hyUserFile = '/etc/hysteria/users.txt';
      const hyCfgFile  = '/etc/hysteria/config.json';
      if (fs.existsSync(hyCfgFile) && fs.existsSync(hyUserFile)) {
        let hlines = fs.readFileSync(hyUserFile, 'utf8').split('\n').filter(l => l.trim() && !l.startsWith(`${username}|`));
        hlines.push(`${username}|${password}|${exp}`);
        fs.writeFileSync(hyUserFile, hlines.join('\n') + '\n');
        fs.chmodSync(hyUserFile, 0o600);
        _hysteriaSyncConfig(hyUserFile, hyCfgFile);
        await restartService('hysteria');
        console.log(`[SSH] ✅ Hysteria synchronisé pour ${username}`);
      }
    } catch (eh) { console.error('[SSH] Erreur sync Hysteria:', eh.message); }
...
async function sshRemove(username) {
  try {
    ...
    try {
      const zivpnUserFile = '/etc/zivpn/users.list';
      const zivpnCfgFile  = '/etc/zivpn/config.json';
      if (fs.existsSync(zivpnUserFile)) {
        const zlines = fs.readFileSync(zivpnUserFile, 'utf8').split('\n').filter(l => l.trim() && !l.startsWith(`${username}|`));
        fs.writeFileSync(zivpnUserFile, zlines.join('\n') + '\n');
        _zivpnSyncConfig(zivpnUserFile, zivpnCfgFile);
        await restartService('zivpn');
      }
    } catch (ez) { console.error('[SSH] Erreur nettoyage ZIVPN:', ez.message); }
    // Nettoyage Hysteria
    try {
      const hyUserFile = '/etc/hysteria/users.txt';
      const hyCfgFile  = '/etc/hysteria/config.json';
      if (fs.existsSync(hyUserFile)) {
        const hlines = fs.readFileSync(hyUserFile, 'utf8').split('\n').filter(l => l.trim() && !l.startsWith(`${username}|`));
        fs.writeFileSync(hyUserFile, hlines.join('\n') + '\n');
        _hysteriaSyncConfig(hyUserFile, hyCfgFile);
        await restartService('hysteria');
      }
    } catch (eh) { console.error('[SSH] Erreur nettoyage Hysteria:', eh.message); }
  } catch {}
}
...
async function sshLock(username) {
  try {
    ...
    // Retirer aussi des configs ZIVPN et Hysteria (bloquer l'accès UDP aussi)
    try {
      const zivpnUserFile = '/etc/zivpn/users.list';
      const zivpnCfgFile  = '/etc/zivpn/config.json';
      if (fs.existsSync(zivpnUserFile)) {
...
function getVpsDomain() {
  ...
  const direct = readFile('/etc/kighmu/domain.txt');
  if (direct) return direct;
  // Fallback ~/.kighmu_info (écrit par menu1.sh et install-1.sh corrigé)
  try {
    const info = fs.readFileSync(`${process.env.HOME || '/root'}/.kighmu_info`, 'utf8');
    const m = info.match(/^DOMAIN=(.+)$/m);
    if (m && m[1].trim()) return m[1].trim();
  } catch {}
  return readFile('/etc/xray/domain') || readFile('/tmp/.xray_domain') || null;
}
    ...
    try {
      ...
      const [resellersWithQuota] = await db.query(`
        SELECT r.id, r.username, r.data_limit_gb,
               COALESCE(SUM(u.upload_bytes+u.download_bytes),0) as total_bytes
        FROM resellers r LEFT JOIN usage_stats u ON u.reseller_id=r.id
        WHERE r.data_limit_gb>0 AND r.is_active=1 GROUP BY r.id`);
