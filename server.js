...
async function addTunnel(client) {
  ...
  try {
    switch (tunnel_type) {
      ...
      case 'udp-zivpn':      result = zivpnAdd(username, password, expires_at); if (result.ok) await restartService('zivpn'); break;
      case 'udp-hysteria':   result = hysteriaAdd(username, password, expires_at); if (result.ok) await restartService('hysteria'); break;
...
async function removeTunnel(client) {
  ...
  try {
    switch (tunnel_type) {
      ...
      case 'ssh-ws':
      case 'ssh-slowdns':
      case 'ssh-ssl':       await sshRemove(username); break;
