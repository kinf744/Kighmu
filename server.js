...
app.get('/api/admin/resellers', A, async (req, res) => {
  try {
    ...
    const parsed = rows.map(r => ({...r, allowed_tunnels: r.allowed_tunnels ? JSON.parse(r.allowed_tunnels) : null}));
    res.json(parsed);
...
app.post('/api/admin/resellers', A, async (req, res) => {
  try {
    ...
    if (allowed_tunnels && Array.isArray(allowed_tunnels) && allowed_tunnels.length > 0) {
      const VALID = ['vless','vmess','trojan','ssh-multi','ssh-ws','ssh-slowdns','ssh-ssl','ssh-udp','udp-zivpn','udp-hysteria','v2ray-fastdns'];
...
app.get('/api/reseller/me', R, async (req, res) => {
  try {
    ...
    // Parser allowed_tunnels
    if (r && r.allowed_tunnels) {
...
app.post('/api/reseller/clients', R, async (req, res) => {
  try {
    ...
    if (r.allowed_tunnels) {
      const allowed = JSON.parse(r.allowed_tunnels);
      if (!allowed.includes(tunnel_type)) {
