...
app.post('/api/reseller/clients', R, async (req, res) => {
  try {
    ...
    const vpsInfo = {
      domain:      getVpsDomain() || null,
      xray_domain: readFile('/etc/xray/domain') || readFile('/tmp/.xray_domain') || null,
      v2ray_domain:readFile('/.v2ray_domain') || null,
    };
    try { vpsInfo.host_ip = (await execAsync("curl -s --max-time 3 ifconfig.me || hostname -I | awk '{print $1}'")).trim(); } catch {}
    res.json({ id: ins.insertId, username, password: pass, uuid, tunnel_type, expires_at, data_limit_gb: data_limit_gb||0, tunnelResult, vpsInfo });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
