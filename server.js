...
function xrayAdd(username, protocol, uuid, expiresAt, limitGb = 0) {
  ...
  // Vérifier doublon
  const alreadyInCfg = protocol === 'trojan'
    ? inb.settings.clients.some(c => c.password === username)
    : inb.settings.clients.some(c => c.id === uuid);
  if (!alreadyInCfg) {
  ...
  // ── 2. Mise à jour users.json (pour compatibilité scripts VPS) ─
  try {
    let users = { vmess: [], vless: [], trojan: [] };
    if (fs.existsSync(usersPath)) {
      const existing = readJson(usersPath);
      if (existing) users = { vmess: [], vless: [], trojan: [], ...existing };
    }
    ...
    if (!users[proto]) users[proto] = [];

    // Vérifier doublon dans users.json
    const alreadyInUsers = proto === 'trojan'
      ? users[proto].some(u => u.password === username)
      : users[proto].some(u => u.uuid === uuid);

    if (!alreadyInUsers) {
      const tag = `${proto}_${username}_${uuid.slice(0, 8)}`;
      const entry = proto === 'trojan'
        ? { password: username, name: username, tag, limit_gb: limitGb, used_gb: 0, expire: expDate }
        : { uuid, name: username, tag, limit_gb: limitGb, used_gb: 0, expire: expDate };
      users[proto].push(entry);
      writeJson(usersPath, users);

      // Mettre à jour aussi users_expiry.list (utilisé par les scripts VPS)
      const expiryPath = '/etc/xray/users_expiry.list';
      try {
        fs.appendFileSync(expiryPath, `${uuid}|${expDate}\n`);
      } catch {}
    }
  } catch (e) {
    console.error('[XRAY] Erreur mise à jour users.json:', e.message);
    // Non bloquant : config.json est déjà mis à jour
  }
  ...
  try {
    if (fs.existsSync(usersPath)) {
      ...
      if (users) {
        const proto = protocol.toLowerCase();
        if (users[proto]) {
          users[proto] = proto === 'trojan'
            ? users[proto].filter(u => u.password !== username && u.name !== username)
            : users[proto].filter(u => u.uuid !== uuid && u.name !== username);
          writeJson(usersPath, users);
        }
      }
    }
  } catch (e) { console.error('[XRAY] Erreur suppression users.json:', e.message); }

  // ── 3. Supprimer de users_expiry.list ───────────────────────
  try {
    const expiryPath = '/etc/xray/users_expiry.list';
    if (fs.existsSync(expiryPath)) {
      const lines = fs.readFileSync(expiryPath, 'utf8').split('\n')
        .filter(l => l.trim() && !l.startsWith(`${uuid}|`));
      fs.writeFileSync(expiryPath, lines.join('\n') + '\n');
    }
  } catch {}
}
...
function v2rayAdd(username, uuid, expiresAt) {
  const cfgPath   = process.env.V2RAY_CONFIG || '/etc/v2ray/config.json';
  const usersPath = '/etc/v2ray/utilisateurs.json';

  // ── 1. Mise à jour config.json (pour V2Ray actif) ──────────
  const cfg = readJson(cfgPath);
  ...
  // ── 2. Mise à jour utilisateurs.json (pour compatibilité scripts VPS) ─
  try {
    let users = [];
    if (fs.existsSync(usersPath)) {
      const existing = readJson(usersPath);
    ...
    // Vérifier doublon
    if (!users.some(u => u.uuid === uuid || u.nom === username)) {
      users.push({ nom: username, uuid, expire: expDate });
      // Écriture sécurisée (même logique que le script bash : mktemp + mv)
      const tmpPath = `/tmp/v2ray_users_${Date.now()}.json`;
    ...
    console.error('[V2RAY] Erreur mise à jour utilisateurs.json:', e.message);
    // Non bloquant : config.json est déjà mis à jour
  }
...
function v2rayRemove(username, uuid) {
  ...
  // ── 2. Supprimer de utilisateurs.json ───────────────────────
  try {
    if (fs.existsSync(usersPath)) {
      const users = readJson(usersPath);
      if (Array.isArray(users)) {
        const filtered = users.filter(u => u.uuid !== uuid && u.nom !== username);
        const tmpPath = `/tmp/v2ray_users_${Date.now()}.json`;
  ...
  } catch (e) { console.error('[V2RAY] Erreur suppression utilisateurs.json:', e.message); }
}
...
async function addTunnel(client) {
  ...
  try {
    switch (tunnel_type) {
      case 'vless':          result = xrayAdd(username, 'vless', uuid, expires_at);  if (result.ok) await restartService('xray');     break;
      case 'vmess':          result = xrayAdd(username, 'vmess', uuid, expires_at);  if (result.ok) await restartService('xray');     break;
      case 'trojan':         result = xrayAdd(username, 'trojan', uuid, expires_at); if (result.ok) await restartService('xray');     break;
      ...
      case 'v2ray-fastdns':  result = v2rayAdd(username, uuid, expires_at);          if (result.ok) await restartService('v2ray');    break;
    }
