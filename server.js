...
app.post('/api/admin/resellers', A, async (req, res) => {
  try {
    const { username, password, email, max_users, expires_at, data_limit_gb } = req.body;
    ...
    const [r] = await db.query(
      'INSERT INTO resellers (username,password,max_users,expires_at,data_limit_gb,created_by) VALUES (?,?,?,?,?,?,?)',
      [username, hash, max_users||10, expires_at, data_limit_gb||0, req.user.id]);
...
app.put('/api/admin/resellers/:id', A, async (req, res) => {
  try {
    const { username, max_users, expires_at, is_active, password, data_limit_gb } = req.body;
    const upd = {};
