    ...
    try {
      ...
      if (decoded.role === 'reseller') {
        ...
        if (new Date(r.expires_at) < new Date()) {
          // Nettoyage complet + suppression du compte expiré
...
function startCron() {
  ...
  cron.schedule('0 * * * *', async () => {
    ...
    try {
      ...
      // Inclure is_active=0 aussi (expirés non encore supprimés)
      ...
      for (const r of expiredResellers) {
        console.log(`[CRON] Revendeur expiré: ${r.username} (#${r.id}) — nettoyage + suppression...`);
        await cleanupReseller(r.id);
        await db.query('DELETE FROM resellers WHERE id=?', [r.id]);
      ...
      if (expiredResellers.length) console.log(`[CRON] ${expiredResellers.length} revendeur(s) expiré(s) supprimés automatiquement`);
