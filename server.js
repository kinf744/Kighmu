    ...
    try {
      ...
      if (decoded.role === 'reseller') {
        ...
        if (new Date(r.expires_at) < new Date()) {
          // Nettoyage complet automatique à l'expiration
          await cleanupReseller(decoded.id);
          return res.status(401).json({ error: 'Compte expiré — données nettoyées automatiquement' });
        }
      }
      req.user = decoded;
      next();
...
async function sshAdd(username, password, expiryDate) {
  try {
    ...
    try {
      ...
      const domain    = readF('/etc/kighmu/domain.txt') || readF('/etc/xray/domain') || readF('/tmp/.xray_domain') || '';
      ...
      const limite  = Math.round((expDate - today) / (1000 * 60 * 60 * 24));
      let lines = [];
    ...
    } catch (e2) { console.error('[SSH] Erreur ecriture /etc/kighmu/users.list:', e2.message); }
    return { ok: true };
...
// Lock SSH user (disable login without deleting)
...
// Block tunnel (désactive accès VPN sans supprimer de la DB)
async function blockTunnel(client) {
  ...
  try {
    switch (tunnel_type) {
      ...
      case 'trojan':
      case 'v2ray-fastdns':
        // Pour Xray/V2Ray: supprimer le client de la config = tunnel bloqué
      ...
      case 'ssh-ssl':
      case 'ssh-udp':
        await sshLock(username); // verrouille le compte SSH
        break;
      case 'udp-zivpn':
        zivpnRemove(username); // retire de la liste active
        break;
      case 'udp-hysteria':
        hysteriaRemove(username); await restartService('hysteria'); break;
    }
    console.log(`[QUOTA] Tunnel bloqué : ${username} (${tunnel_type})`);
  } catch (e) { console.error(`[QUOTA] blockTunnel error:`, e.message); }
}

// Débloquer tunnel (réactiver après recharge quota)
async function unblockTunnel(client) {
  ...
  try {
    switch (tunnel_type) {
      ...
      case 'udp-zivpn':
        zivpnAdd(username, password, expires_at); break;
      case 'udp-hysteria':
        hysteriaAdd(username, password, expires_at); await restartService('hysteria'); break;
    }
...
// ============================================================
// NETTOYAGE COMPLET REVENDEUR (expiration ou suppression)
// ============================================================
async function cleanupReseller(resellerId, logActorId) {
  try {
    // 1. Récupérer tous les clients du revendeur
    const [clients] = await db.query('SELECT * FROM clients WHERE reseller_id=?', [resellerId]);

    // 2. Supprimer tous les tunnels VPN
    for (const c of clients) {
      await removeTunnel(c);
    }

    // 3. Supprimer toutes les données en cascade
    ...
    // 4. Réinitialiser les compteurs du revendeur
...
app.post('/api/auth/reseller/login', async (req, res) => {
  try {
    ...
    if (new Date(r.expires_at) < new Date()) {
      // Nettoyage complet au login si expiré
...
app.post('/api/admin/resellers', A, async (req, res) => {
  try {
    ...
    // Valider le JSON des tunnels autorisés
    let tunnelList = null;
    if (allowed_tunnels && Array.isArray(allowed_tunnels) && allowed_tunnels.length > 0) {
      // SSH: on stocke uniquement 'ssh-multi' qui couvre toutes variantes SSH
    ...
    const allowed_tunnels_json = tunnelList;
    ...
    const [r] = await db.query(
      ...
      [username, hash, max_users||10, expires_at, data_limit_gb||0, allowed_tunnels_json, req.user.id]);
  ...
  try {
    ...
    if (allowed_tunnels !== undefined) {
      if (Array.isArray(allowed_tunnels) && allowed_tunnels.length > 0) {
        ...
        upd.allowed_tunnels = null; // null = tous les tunnels autorisés
      }
    }
...
// SUPPRESSION REVENDEUR — nettoyage complet
...
// CLEAN MANUEL (sans supprimer le revendeur)
...
app.post('/api/admin/clients', A, async (req, res) => {
  try {
    ...
    const readF = p => { try { return fs.readFileSync(p,'utf8').trim(); } catch { return null; } };
    const vpsInfo = {
      domain:      readF('/etc/kighmu/domain.txt') || readF('/etc/xray/domain') || readF('/tmp/.xray_domain') || null,
...
app.put('/api/admin/clients/:id', A, async (req, res) => {
  try {
    ...
    // Mise à jour du tunnel VPN si username/password/uuid changés
    if (tunnelChanged) {
      const updated = { ...c, ...upd };
      await removeTunnel(c);                          // supprimer l'ancienne config
      await addTunnel(updated);                       // recréer avec nouvelles infos
    }
...
app.get('/api/reseller/me', R, async (req, res) => {
  try {
    ...
    // Parser allowed_tunnels — toujours retourner un array ou null
    if (r) {
      if (r.allowed_tunnels && typeof r.allowed_tunnels === 'string') {
        try { r.allowed_tunnels = JSON.parse(r.allowed_tunnels); } catch { r.allowed_tunnels = null; }
      }
      // S'assurer que c'est bien un array non vide, sinon null
      if (!Array.isArray(r.allowed_tunnels) || r.allowed_tunnels.length === 0) {
        r.allowed_tunnels = null;
      }
    }
    res.json(r);
  } catch (e) { res.status(500).json({ error: e.message }); }
...
app.post('/api/reseller/clients', R, async (req, res) => {
  try {
    ...
    // Vérifier que le tunnel est autorisé pour ce revendeur
    if (r.allowed_tunnels) {
      ...
      if (allowed) {
        // ssh-multi dans allowed_tunnels couvre toutes les variantes SSH
...
app.get('/api/admin/vps-info', A, async (req, res) => {
  try {
    ...
    const domain = readFile('/etc/kighmu/domain.txt') || readFile('/etc/xray/domain') || readFile('/tmp/.xray_domain') || readFile('/etc/hysteria/domain.txt') || readFile('/etc/zivpn/domain.txt') || null;
    let hostIp = null;
    try { const nets = await si.networkInterfaces(); const eth = nets.find(n => !n.internal && n.ip4); hostIp = eth?.ip4 || null; } catch {}
...
app.get('/api/reseller/vps-info', R, async (req, res) => {
  const readFile = (p) => { try { return fs.readFileSync(p,'utf8').trim(); } catch { return null; } };
  const domain = readFile('/etc/kighmu/domain.txt') || readFile('/etc/xray/domain') || readFile('/tmp/.xray_domain') || null;
  let hostIp = null;
  try { const nets = await si.networkInterfaces(); const eth = nets.find(n=>!n.internal&&n.ip4); hostIp=eth?.ip4||null; } catch {}
  const slowdnsKey = readFile('/etc/slowdns/server.pub') || readFile('/etc/slowdns_v2ray/server.pub') || null;
  const slowdnsNs  = readFile('/etc/slowdns/ns.conf') || readFile('/etc/slowdns_v2ray/ns.conf') || null;
...
// ============================================================
// ROUTE RAPPORT TRAFIC — appelée par script VPS toutes les X min
// Sécurisée par header x-report-secret (= REPORT_SECRET dans .env)
// ============================================================
app.post('/api/report/traffic', async (req, res) => {
  try {
    ...
    // stats = [{ username, upload_bytes, download_bytes }]
    ...
    for (const s of stats) {
      ...
      const [[ex]] = await db.query(
        'SELECT id FROM usage_stats WHERE client_id=? ORDER BY recorded_at DESC LIMIT 1', [c.id]
      ).catch(() => [[null]]);

      if (ex) {
        await db.query(
          'UPDATE usage_stats SET upload_bytes=upload_bytes+?, download_bytes=download_bytes+?, recorded_at=NOW() WHERE id=?',
          [up, down, ex.id]
        );
      } else {
        await db.query(
          'INSERT INTO usage_stats (client_id, reseller_id, upload_bytes, download_bytes) VALUES (?,?,?,?)',
          [c.id, c.reseller_id || null, up, down]
        );
      }
      updated++;
    }
...
// Route SET (remplace au lieu d'ajouter) — pour des outils comme vnstat
...
// ============================================================
function startCron() {
  // Toutes les heures: expiration clients + expiration revendeurs + vérif quota
  cron.schedule('0 * * * *', async () => {
    if (!db) return;
    try {
      // 1. Expiration revendeurs → nettoyage complet
      const [expiredResellers] = await db.query('SELECT id, username FROM resellers WHERE expires_at < NOW() AND is_active=1');
      for (const r of expiredResellers) {
        console.log(`[CRON] Revendeur expiré: ${r.username} — nettoyage complet...`);
        await cleanupReseller(r.id);
        await db.query('UPDATE resellers SET is_active=0 WHERE id=?', [r.id]);
      }
      if (expiredResellers.length) console.log(`[CRON] ${expiredResellers.length} revendeur(s) expiré(s) et nettoyés`);

      // 2. Expiration clients → suppression tunnel
      ...
      // 3. Vérification quota data clients
      const [clientsWithQuota] = await db.query(`
        ...
        FROM clients c
        LEFT JOIN usage_stats u ON u.client_id = c.id
        WHERE c.data_limit_gb > 0 AND c.is_active = 1
        GROUP BY c.id`);
      ...
      for (const c of clientsWithQuota) {
        ...
        if (usedGb >= limitGb && !c.quota_blocked) {
          // Quota dépassé — bloquer le tunnel
          await blockTunnel(c);
          await db.query('UPDATE clients SET quota_blocked=1, is_active=0 WHERE id=?', [c.id]);
          console.log(`[CRON] Quota dépassé → tunnel bloqué: ${c.username} (${usedGb.toFixed(2)}GB / ${limitGb}GB)`);
        }
      }

      // 4. Vérification quota data revendeurs
      const [resellersWithQuota] = await db.query(`
        ...
        WHERE r.data_limit_gb > 0 AND r.is_active = 1
        GROUP BY r.id`);
      ...
      for (const r of resellersWithQuota) {
        ...
        if (usedGb >= limitGb) {
          // Quota revendeur dépassé — bloquer tous ses clients actifs
          const [rClients] = await db.query('SELECT * FROM clients WHERE reseller_id=? AND is_active=1', [r.id]);
          for (const c of rClients) {
            await blockTunnel(c);
            await db.query('UPDATE clients SET quota_blocked=1, is_active=0 WHERE id=?', [c.id]);
          }
          console.log(`[CRON] Quota revendeur dépassé: ${r.username} — ${rClients.length} client(s) bloqué(s)`);
        }
      }
  ...
  // Tous les jours: nettoyer les tentatives de login expirées
