...
async function snapshotClientTraffic(clientId) {
  try {
    const ym = new Date().toISOString().slice(0, 7);
    // Récupérer les infos du client (username et tunnel_type)
    const [[client]] = await db.query(
      "SELECT username, tunnel_type FROM clients WHERE id=?",
      [clientId]
    );
    if (!client) return;
    ...
    // Sauvegarder les données SPÉCIFIQUES à ce username (SOLUTION AU PROBLÈME DE RÉUTILISATION)
    await db.query(
      "INSERT INTO user_traffic_snapshot (username, tunnel_type, ym, upload_bytes, download_bytes, deleted_at) VALUES (?, ?, ?, ?, ?, NOW()) ON DUPLICATE KEY UPDATE upload_bytes=VALUES(upload_bytes), download_bytes=VALUES(download_bytes), deleted_at=NOW()",
      [client.username, client.tunnel_type, ym, up, dn]
    );
    
    // Sauvegarder aussi dans le snapshot global pour compatibilité
    await db.query(
      "INSERT INTO monthly_traffic_snapshot (ym, upload_bytes, download_bytes) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE upload_bytes=upload_bytes+VALUES(upload_bytes), download_bytes=download_bytes+VALUES(download_bytes), updated_at=NOW()",
      [ym, up, dn]
    );
...
// Nettoyer les anciens snapshots après 90 jours pour éviter les réutilisations accidentelles
async function cleanupOldSnapshots() {
  try {
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - 90);
    const result = await db.query(
      "DELETE FROM user_traffic_snapshot WHERE deleted_at < ?",
      [cutoffDate.toISOString().slice(0, 10)]
    );
    if (result[0].affectedRows > 0) {
      console.log(`[CLEANUP] ${result[0].affectedRows} anciens snapshots d'utilisateurs supprimés`);
    }
  } catch(e) { console.error("[CLEANUP] cleanupOldSnapshots:", e.message); }
}
...
app.post('/api/admin/clients', A, async (req, res) => {
  try {
    ...
    // Vérifier que ce username n'a pas été supprimé récemment (prévenir la réutilisation de données)
    const [[snapshot]] = await db.query(
      'SELECT username FROM user_traffic_snapshot WHERE username=? LIMIT 1',
      [username]
    );
    if (snapshot) {
      return res.status(409).json({ 
        error: `Username '${username}' a été supprimé récemment. Attendez 24h ou choisissez un autre nom.`,
        warning: 'Pour éviter l\'accumulation de données anciennes'
      });
    }
...
app.post('/api/reseller/clients', R, async (req, res) => {
  try {
    ...
    // Vérifier que ce username n'a pas été supprimé récemment
    const [[snapshot]] = await db.query(
      'SELECT username FROM user_traffic_snapshot WHERE username=? LIMIT 1',
      [username]
    );
...
function startCron() {
  ...
  // + Nettoyage des anciens snapshots d'utilisateurs supprimés
  cron.schedule('5 0 1 * *', async () => {
    ...
    try {
      ...
      // Nettoyer les anciens snapshots d'utilisateurs (>90 jours)
