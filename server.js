...
async function sshAdd(username, password, expiryDate) {
  ...
  try {
    // exp = date ISO pour la DB et users.list
    const exp = new Date(expiryDate).toISOString().split('T')[0];
    // expForChage = lendemain si exp est aujourd'hui ou passé
    // (chage -E expire le compte à MINUIT → si exp=aujourd'hui, compte expiré immédiatement)
    const _expD = new Date(exp + 'T00:00:00');
    const _todD = new Date(); _todD.setHours(0,0,0,0);
    const expForChage = _expD <= _todD
      ? new Date(Date.now() + 86400000).toISOString().split('T')[0]
      : exp;
    const readF = p => { try { return fs.readFileSync(p, 'utf8').trim(); } catch { return ''; } };

    // Lire DOMAIN (ordre : domain.txt → ~/.kighmu_info → xray/domain)
    const getDomain = () => {
      const d = readF('/etc/kighmu/domain.txt');
      if (d) return d;
      try {
      ...
      return readF('/etc/xray/domain') || readF('/tmp/.xray_domain') || '';
    };
    ...
    const zivpnUF     = '/etc/zivpn/users.list';
    const zivpnCfg    = '/etc/zivpn/config.json';
    const hyUF        = '/etc/hysteria/users.txt';
    const hyCfg       = '/etc/hysteria/config.json';
...
`;
    ...
    // ── ZIVPN sync ───────────────────────────────────────────────
    try {
      if (fs.existsSync(zivpnCfg) && fs.existsSync(zivpnUF)) {
        let zl = fs.readFileSync(zivpnUF, 'utf8').split('\n').filter(l => l.trim() && !l.startsWith(`${username}|`));
        zl.push(`${username}|${password}|${exp}`);
        fs.writeFileSync(zivpnUF, zl.join('\n') + '\n');
        fs.chmodSync(zivpnUF, 0o600);
        _zivpnSyncConfig(zivpnUF, zivpnCfg);
    ...
    // ── Hysteria sync ────────────────────────────────────────────
    try {
      if (fs.existsSync(hyCfg) && fs.existsSync(hyUF)) {
        let hl = fs.readFileSync(hyUF, 'utf8').split('\n').filter(l => l.trim() && !l.startsWith(`${username}|`));
        hl.push(`${username}|${password}|${exp}`);
        fs.writeFileSync(hyUF, hl.join('\n') + '\n');
        fs.chmodSync(hyUF, 0o600);
        _hysteriaSyncConfig(hyUF, hyCfg);
