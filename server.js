...
async function sshAdd(username, password, expiryDate) {
  try {
    ...
    // 1. Creer l'utilisateur Linux
    await execAsync(`useradd -M -s /bin/false -e ${exp} ${username} 2>/dev/null || true`);
    await execAsync(`echo "${username}:${password}" | chpasswd`);

    // 2. Ecrire dans /etc/kighmu/users.list (format: user|pass|limite|expire|hostip|domain|slowdns_ns)
    try {
      ...
      if (fs.existsSync(userFile)) {
        lines = fs.readFileSync(userFile, 'utf8')
          .split('\n')
      ...
      console.error('[SSH] Erreur ecriture /etc/kighmu/users.list:', e2.message);
    }
...
async function sshRemove(username) {
  try {
    ...
    try {
      ...
      if (fs.existsSync(userFile)) {
        const lines = fs.readFileSync(userFile, 'utf8')
          .split('\n')
          .filter(l => l.trim() && !l.startsWith(`${username}|`));
        fs.writeFileSync(userFile, lines.join('\n') + '\n');
      }
    } catch (e2) {
      console.error('[SSH] Erreur suppression /etc/kighmu/users.list:', e2.message);
    }
  } catch {}
}
...
app.post('/api/admin/clients', A, async (req, res) => {
  try {
    ...
    const dataLimitGb = parseFloat(data_limit_gb) || 0;
    const [ins] = await db.query(
      'INSERT INTO clients (username,password,uuid,reseller_id,tunnel_type,expires_at,note,data_limit_gb) VALUES (?,?,?,?,?,?,?,?)',
      [username, pass, uuid, reseller_id||null, tunnel_type, expires_at, note||null, dataLimitGb]);
...
app.put('/api/admin/clients/:id', A, async (req, res) => {
  try {
    ...
    if (is_active      !== undefined) upd.is_active      = is_active;
    if (data_limit_gb  !== undefined) upd.data_limit_gb  = parseFloat(data_limit_gb) || 0;
    if (!Object.keys(upd).length) return res.status(400).json({ error: 'Aucun champ à modifier' });
    const sets = Object.keys(upd).map(k => `${k}=?`).join(',');
...
app.post('/api/reseller/clients', R, async (req, res) => {
  try {
    ...
    const [ins] = await db.query(
      'INSERT INTO clients (username,password,uuid,reseller_id,tunnel_type,expires_at,note,data_limit_gb) VALUES (?,?,?,?,?,?,?,?)',
      [username, pass, uuid, req.user.id, tunnel_type, expires_at, note||null, dataLimitGb]);
